...
class AICBGlobalCache:
    ...
    def print_stats(self) -> None:
        ...
        logger.info(f"\n{'='*70}")
        logger.info(f"[AICB Cache Stats Report (统计报告)]")
        logger.info(f"{'='*70}")
        logger.info(f"  Cache entries (缓存条目数):     {len(self._cache)}")
        logger.info(f"  Exact hits (精确命中, 查表): {self._stats['cache_hits']}")
        logger.info(f"  Interpolated hits (插值命中):       {self._stats['interpolations']}")
        logger.info(f"  AICB real calls (AICB实际调用):   {self._stats['aicb_calls']}")
        logger.info(f"  CSV file loads (CSV文件加载):    {self._stats['csv_loads']}")
        if total_queries > 0:
            hit_rate = (self._stats['cache_hits'] + self._stats['interpolations']) / total_queries * 100
            logger.info(f"  Cache hit rate (缓存命中率):     {hit_rate:.1f}%")
        logger.info(f"  Cache dir (缓存目录):       {_AICB_CACHE_DIR}")
        logger.info(f"  Log file (日志文件):       {self._log_file}")
        logger.info(f"{'='*70}\n")
...
class ExecutionTime(BaseEntity):
    ...
    def _get_attention_layer_execution_time_from_aicb(self,layer_id) -> float:
        ...
        attention_time = attention_comp_time + 0 # TODO(tianhao909): add attention_comm_time
    ...
    def _get_mlp_layer_execution_time_from_aicb(self, layer_id) -> float:
        ...
        mlp_time = mlp_comp_time + 0 # TODO(tianhao909): add mlp_comm_time
    ...
    def _get_moe_layer_execution_time_from_aicb(self, layer_id) -> float:
        ...
        replica_stage = "prefill" # TODO(tianhao909): determine stage from runtime context
    ...
    def _get_aicb_params(self):
        ...
        # TODO(tianhao909): use real values for bs/seq/phase alignment
        # TODO(tianhao909): 把这几个加进去，用真实的值对齐
        ...
        try:
            ...
            if results_dir.exists():
                ...
                if os.path.exists(expected_csv):
                    logger.info(f'SUCCESS: Expected CSV file was created!')
                else:
    ...
    def _generate_or_find_bs1_csv(
        ...
        if bs == 1:
            ...
            if self._generate_aicb_csv():
                if os.path.exists(original_csv_path):
                    logger.info(f'[AICB Safe Mode] 成功生成 bs=1 CSV: {original_csv_path}')
        ...
        if gen_ok and os.path.exists(bs1_csv_path):
            logger.info(f'[AICB Safe Mode] 成功生成 bs=1 CSV: {bs1_csv_path}')
            return bs1_csv_path
        else:
    ...
    def _load_aicb_data(self) -> Dict[int, Dict[str, Dict[str, float]]]:
        ...
        if not os.path.exists(full_csv_path):
            ...
            # [AICB Safe Mode] 始终使用 bs=1 生成/查找 CSV
            #
            # Reason: AICB's per_token_group_quant_fp8 CUDA kernel
            #         is incompatible with bs>1 on some GPUs (e.g. H20)
            # 原因: AICB 的 per_token_group_quant_fp8 等 CUDA kernel
            #       在某些 GPU (如 H20) 上对 bs>1 不兼容
            #
            # Strategy: Use bs=1 CSV because:
            # 策略: 直接使用 bs=1 生成 CSV, 因为:
            #   1. AICB measures per-token CUDA kernel time
            #      AICB 测量的是 per-token CUDA kernel 耗时
            #   2. Per-token time is largely independent of batch size
            #      单 token 耗时与 batch size 基本无关
            #   3. bs=1 has simplest kernel config, best GPU compatibility
            #      bs=1 的 kernel 配置最简单, GPU 兼容性最好
            # ============================================================
            ...
            if not os.path.exists(full_csv_path):
                ...
                if matches1:
                    ...
                    logger.info(f'[AICB Fallback] 找到同模型同ws: {full_csv_path}')
                    found_fallback = True
                ...
                if not found_fallback:
                    ...
                    if matches2:
                        ...
                        logger.info(f'[AICB Fallback] 找到同模型: {full_csv_path}')
                        found_fallback = True
                ...
                if not found_fallback:
                    ...
                    if all_csvs:
                        ...
                        logger.info(f'[AICB Fallback] 使用任意可用CSV: {full_csv_path}')
                        found_fallback = True
    ...
    def _preload_decode_endpoint(self, last_seq: int):
        ...
        logger.info(f"[AICB] Preloading decode last round (开始预加载decode最后一轮): "
        ...
        if not os.path.exists(csv_path):
            ...
            # [AICB Safe Mode] 使用 bs=1 生成, 避免 CUDA kernel 兼容性问题
            # ============================================================
            logger.debug(f"[AICB首尾插值] last_seq CSV不存在: {csv_path}")
        ...
        if os.path.exists(csv_path):
            ...
            if data:
                ...
                logger.info(f"[AICB首尾插值] 成功缓存last_seq={last_seq}, layers={len(data)}")
    ...
    def _parse_aicb_csv(self, csv_path: str) -> Dict[int, Dict[str, Dict[str, float]]]:
        ...
        logger.info(f"[AICB优化] 成功解析CSV: {len(data)} layers from {csv_path}")
        return data
    ...
    def model_time(self) -> float:
        ...
        if self._replica_config.model_name in ['deepseek-671B', 'qwen3-moe-235B', 'qwen3-next-80B'] and self._config.backend == 'aicb':
            ...
            # TODO(tianhao909): locate _pipeline_stage_id and integrate with batch id
            # TODO(tianhao909): 找 _pipeline_stage_id 在哪，结合 batch id
