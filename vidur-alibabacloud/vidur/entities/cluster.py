...
class Cluster(BaseEntity):
    ...
    def __init__(
        ...
        if rc.pd_node_ratio == 1:
            ...
            logger.info(f"{'='*70}")
            logger.info(f"[Cluster] PD off, MIXED mode (PD 分离关闭, pd_node_ratio=1, MIXED 模式)")
            logger.info(f"[Cluster] tp={rc.tensor_parallel_size}, pp={rc.num_pipeline_stages}, "
                        f"dp={dp}, ws={full_ws}, ep={full_ws}")
            logger.info(f"[Cluster] prefill_ws={rc.prefill_world_size}, decode_ws={rc.decode_world_size} (same, 相同)")
            logger.info(f"{'='*70}")
        ...
        # ============================================================
            ...
            # --- 详尽打印 PD 配置 ---
            logger.info(f"{'='*70}")
            logger.info(f"[PD Config] PD enabled (PD 分离开启) ({replica_source})")
            logger.info(f"[PD Config] Total replicas (总 replica 数): {num_replicas} "
                        f"(prefill={num_p}, decode={num_d})")
            logger.info(f"[PD Config] Prefill: tp={p_tp}, pp={p_pp}, dp={num_p}, "
                        f"ws={rc.prefill_world_size}, ep={rc.prefill_ep}")
            logger.info(f"[PD Config] Decode:  tp={d_tp}, pp={d_pp}, dp={num_d}, "
