...
class Primitives<
  ...
  // Recv/Send - whether need recv/send data
  // SrcBuf - If input is 0, srcs only one element, which is Prev GPU buffer
  //          If not 0, then first element in srcs if srcIx, second element is Prev GPU buffer.
  ...
  __device__ __forceinline__ void genericOp(
    ...
    // stepSize = ncclShmem.comm.buffSizes[NCCL_PROTO_SIMPLE]/NCCL_STEPS/sizeof(T), maybe called stepCount
    int sliceSize = stepSize*StepPerSlice;
    sliceSize = max(divUp(nelem, 16*SlicePerChunk)*16, sliceSize/32); // sliceCount
    int slice = 0;
    int offset = 0;
