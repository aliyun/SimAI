...
ncclResult_t ncclTopoSearchInit(struct ncclTopoSystem* system) {
  ...
  for (int g=0; g<system->nodes[GPU].count; g++) {
    ...
    system->totalBw = std::max(system->totalBw, getTotalBw(system, gpu)); // A100 20GB/s per link, total 12 links
  }
  INFO(NCCL_MOCK_ROOT, "After ncclTopoSearchInit, system_info{nodes[NET].count: %d, nodes[GPU].count: %d, maxBW: %f, totalBW: %f}.",
...
ncclResult_t ncclTopoPrintGraph(struct ncclTopoSystem* system, struct ncclTopoGraph* graph) {
  ...
  INFO(NCCL_MOCK_ROOT, "Pattern %d, crossNic %d, nChannels %d, bw %f/%f, type %s/%s, sameChannels %d", graph->pattern, graph->crossNic, graph->nChannels, graph->bwIntra, graph->bwInter, topoPathTypeStr[graph->typeIntra], topoPathTypeStr[graph->typeInter], graph->sameChannels);
  int ngpus = system->nodes[GPU].count;
