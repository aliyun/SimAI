...
ncclResult_t ncclTopoCheckP2p(struct ncclTopoSystem* system, int64_t id1, int64_t id2, int* p2p, int *read, int* intermediateRank) {
  ...
  int64ToBusId(id1, busId);
  INFO(NCCL_MOCK_ROOT, "In ncclTopoCheckP2p id1: %s", busId);
  *p2p = 0;
  if (read) *read = 0;
...
ncclResult_t ncclTopoTrimSystem(struct ncclTopoSystem* system, struct ncclComm* comm) {
  ...
  // [MOCK Change] Prevent deletion of NET in 1Node situation.
  if (((system->nodes[GPU].count == comm->nRanks) && (comm->nNodes == 0)) || comm->MNNVL || comm->nNodes == 1) {
...
ncclResult_t ncclTopoComputeP2pChannels(struct ncclComm* comm) {
  ...
  for (int c=0; c<comm->p2pnChannels; c++) {
    ...
    TRACE(NCCL_MOCK_ROOT, "comm->p2pChannels[%d]: %d", 
        c, comm->p2pChannels[c]);
  }
  INFO(NCCL_MOCK_ROOT, "comm->p2pnChannels: %d, comm->p2pnChannelsPerPeer: %d", 
