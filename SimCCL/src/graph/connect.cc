...
// Split the entire channel into information corresponding to each rank, and initialize comm->channels
...
static ncclResult_t connectTrees(struct ncclComm* comm, int* treeToParent, int* treeToChild0, int* treeToChild1, int* treePatterns) {
  ...
  for (int c=0; c<nChannels; c++) {
     ...
     if (comm->rank == ttp[node] ||
         comm->rank == ttc0[node] ||
         comm->rank == ttc1[node]) {
      //  INFO(NCCL_MOCK_ROOT, "Grank[%d] Tree %d : %d -> %d -> %d/%d/%d", comm->rank, c,           channel0->tree.up, comm->rank, channel0->tree.down[0], channel0->tree.down[1], channel0->tree.down[2]);
      //  INFO(NCCL_MOCK_ROOT, "Grank[%d] Tree %d : %d -> %d -> %d/%d/%d", comm->rank, c+nChannels, channel1->tree.up, comm->rank, channel1->tree.down[0], channel1->tree.down[1], channel1->tree.down[2]);
...
ncclResult_t ncclTopoPostset(struct ncclComm* comm, int* firstRanks, int* treePatterns, struct ncclTopoRanks** allTopoRanks, int* rings, struct ncclTopoGraph** graphs) {
...
#ifdef MOCK_DEBUG
  if (comm->rank == 0) {
    for (int i = 0; i < nNodes*MAXCHANNELS; i++) {
      INFO(NCCL_MOCK_ROOT, "treeToParent[%d]: %d, treeToChild0[%d]: %d, treeToChild1[%d]: %d", i, treeToParent[i], i, treeToChild0[i], i, treeToChild1[i]);
    }
  }
#endif
  ...
  // Output the ring topology: comm->mockOutput->ringChannels
  if (comm->rank == 0) {
    for (int c = 0; c < nChannels; c++) {
      for (int r = 0; r < nranks; r++) {
        int prev = ringPrev[c * nranks + r];
        int next = ringNext[c * nranks + r];
        int recv = ringRecv[c * nNodes + comm->rankToNode[r]];
        int send = ringSend[c * nNodes + comm->rankToNode[r]];
        comm->mockOutput->ringChannels[c][r] = {prev, next, recv, send};
        TRACE(NCCL_MOCK_ROOT, "nChannels: %d, nranks: %d, rank: %d, ringChannels[%d][%d]: %d %d %d %d\n", nChannels, nranks, comm->rank, c, r, 
              comm->mockOutput->ringChannels[c][r][0],
              comm->mockOutput->ringChannels[c][r][1],
              comm->mockOutput->ringChannels[c][r][2],
              comm->mockOutput->ringChannels[c][r][3]);
      }
    }
  }
