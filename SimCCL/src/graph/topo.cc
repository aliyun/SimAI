...
ncclResult_t ncclTopoPrint(struct ncclTopoSystem* s) {
  ...
  for (int i = 0; i < s->nodes[GPU].count; i++)
    INFO(NCCL_MOCK_ROOT, "s->nodes[GPU].nodes[%d].gpu.rank: %d", i, s->nodes[GPU].nodes[i].gpu.rank);
  return ncclSuccess;
}
...
ncclResult_t ncclTopoGetSystem(struct ncclComm* comm, struct ncclTopoSystem** system) {
  ...
  free(xml);
  //TODO 这里假设单机内所有GPU compCap 相同
  comm->compCap = (*system)->nodes[GPU].nodes[0].gpu.cudaCompCap;
  comm->minCompCap = comm->maxCompCap = comm->compCap;
  if (comm->rank == 0) {
    INFO(NCCL_MOCK_ROOT, "After ncclTopoGetSystem, system_info{nodes[NET].count: %d, nodes[GPU].count: %d, maxBW: %f, totalBW: %f}.",
        (*system)->nodes[NET].count, (*system)->nodes[GPU].count, (*system)->maxBw, (*system)->totalBw);
    for (int i = 0; i < (*system)->nodes[GPU].count; i++) {
      char busId[NVML_DEVICE_PCI_BUS_ID_BUFFER_SIZE];
      int64ToBusId((*system)->nodes[GPU].nodes[i].id, busId);
      INFO(NCCL_MOCK_ROOT, "(*system)->nodes[GPU].nodes[%d].id: %s, gdr = %d", i, busId, (*system)->nodes[GPU].nodes[i].gpu.gdrSupport);
    }
  }
  return ncclSuccess;
}
