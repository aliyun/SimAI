...
struct ncclNodeRanks {
  int localRanks; // num of how many local ranks in this node
  int* localRankToRank; // this node's local rank to global rank, size: localRanks
};
...
struct ncclComm {
  ...
  int node;
  int nNodes;
  int localRank; // get from comm->rankToLocalRank[rank]
  int localRanks; // num of how many local ranks in this node, get from comm->nodeRanks[comm->node].localRanks
  int maxLocalRanks;
  int* rankToNode; // size: nranks, rankToNode[r] = node
  int* rankToLocalRank; // Have global info, size: nranks
  int* localRankToRank;
  // localRanks and localRanktoRank for all nodes, size: nNodes
  struct ncclNodeRanks* nodeRanks; // Have global info!
  ...
  struct ncclMockOutput *mockOutput;
};
