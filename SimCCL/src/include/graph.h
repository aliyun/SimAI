...
struct ncclTopoGraph {
  // Input / output
  int id; // ring : 0, tree : 1, collnet : 2, nvls : 3
  int pattern;
  int crossNic;
  int collNet;
  int minChannels;
  ...
  // Output
  int nChannels;
  float bwIntra; // intra-node channel bandwidth
  float bwInter; // inter-node channel bandwidth
  float latencyInter;
  int typeIntra; // intra-node channel type
  int typeInter; // inter-node channel type
  int sameChannels;
  int nHops;
  int intra[MAXCHANNELS*NCCL_TOPO_MAX_NODES]; // intra node gpu_rank(global) max: 32x256
...
struct ncclTopoRanks {
  ...
  int ringPrev[MAXCHANNELS]; // ring channel receive from which, index means channel
  int ringNext[MAXCHANNELS]; // ring channel send to which, index means channel
