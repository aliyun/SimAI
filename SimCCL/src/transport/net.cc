...
static ncclResult_t sendProxyProgress(struct ncclProxyState* proxyState, struct ncclProxyArgs* args) {
  ...
  if (args->state == ncclProxyOpProgress) {
    ...
    for (int s=0; s<args->nsubs; s++) {
      ...
      // Post buffers to the GPU, This steps is providing the buffer address to the GPU, so that the GPU knows where to write data to. It's only providing a pointer, not the data.
      ...
      if (sub->transmitted < sub->posted && sub->transmitted < sub->done + NCCL_STEPS) {
        ...
        volatile uint64_t* recvTail = &resources->recvMem->tail; // GPU 会去写这个 tail 的指针值
