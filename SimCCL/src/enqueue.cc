...
static ncclResult_t computeCollSteps(struct ncclInfo* collInfo, size_t workCount, uint32_t* steps) {
  ...
  if (collInfo->recvbuff != nullptr) {
    auto& collInfo_entry = static_cast<struct ncclMockOutput *>(collInfo->recvbuff)->collInfos[collInfo->coll][collInfo->nBytes];
    collInfo_entry.algo = collInfo->algorithm;
    collInfo_entry.protocol = collInfo->protocol;
    collInfo_entry.chunkSize = collInfo->chunkSize;
    collInfo_entry.chunkCount = collInfo->chunkCount;
    collInfo_entry.chunkSteps = collInfo->chunkSteps;
    collInfo_entry.sliceSteps = collInfo->sliceSteps;
    collInfo_entry.stepSize = collInfo->stepSize;
    collInfo_entry.workCount = workCount;
    collInfo_entry.steps = *steps;
  }
  INFO(NCCL_MOCK_ROOT, "After computeCollSteps collInfo: coll(%s), chunkSize(%d), chunkCount(%d), chunkSteps(%d), sliceSteps(%d), stepSize(%d), workCount(%zu), steps(%u).", 
                      name_coll[collInfo->coll], collInfo->chunkSize, collInfo->chunkCount, collInfo->chunkSteps, collInfo->sliceSteps, collInfo->stepSize, workCount, *steps);
  return ncclSuccess;
}
...
static ncclResult_t computeCollAlignCount(struct ncclInfo* collInfo, size_t* alignCount) {
  ...
  INFO(NCCL_MOCK_ROOT, "After computeCollAlignCount collInfo: chunkSize(%d), chunkCount(%d), chunkSteps(%d), sliceSteps(%d), stepSize(%d), alignCount(%zu).", 
        collInfo->chunkSize, collInfo->chunkCount, collInfo->chunkSteps, collInfo->sliceSteps, collInfo->stepSize, *alignCount);
  return ncclSuccess;
}
...
static ncclResult_t computeCollLastChunkInfo(struct ncclInfo* collInfo, size_t workCount, size_t alignCount, size_t* lastChunkCount) {
  ...
  if (collInfo->recvbuff != nullptr) {
    auto& collInfo_entry = static_cast<struct ncclMockOutput *>(collInfo->recvbuff)->collInfos[collInfo->coll][collInfo->nBytes];
    collInfo_entry.lastChunkCount = *lastChunkCount;
    collInfo_entry.alignCount = alignCount;
  }

  INFO(NCCL_MOCK_ROOT, "After computeCollLastChunkInfo collInfo: chunkSize(%d), chunkCount(%d), chunkSteps(%d), sliceSteps(%d), stepSize(%d), workCount(%zu), alignCount(%zu), lastChunkCount(%zu).", 
        collInfo->chunkSize, collInfo->chunkCount, collInfo->chunkSteps, collInfo->sliceSteps, collInfo->stepSize, workCount, alignCount, *lastChunkCount);
  return ncclSuccess;
}
...
const char *name_algo[NCCL_NUM_ALGORITHMS] = {
  "NCCL_ALGO_TREE",
  "NCCL_ALGO_RING",
...
const char *name_proto[NCCL_NUM_PROTOCOLS] = {
  "NCCL_PROTO_LL",
  "NCCL_PROTO_LL128",
  "NCCL_PROTO_SIMPLE"
};
...
static ncclResult_t topoGetAlgoInfo(struct ncclInfo* collInfo, int collNetSupport, int nvlsSupport, int numPipeOps) {
  ...
  if (comm->nRanks == 1) {
    ...
    INFO(NCCL_MOCK_ROOT, "Coll Type(\033[32m%s\033[0m) %ld Bytes -> Algo %s proto %s time %f", name_coll[collInfo->coll], collInfo->nBytes, name_algo[collInfo->algorithm], name_proto[collInfo->protocol], minTime);
...
static ncclResult_t getTunerInfo(struct ncclInfo* collInfo, int collNetSupport, int nvlsSupport, int numPipeOps) {
  ...
  if (collInfo->nChannels != 0)
    collInfo->userTuned = true; // Need NCCL_TUNER_PLUGIN
  else
...
static ncclResult_t getChannnelThreadInfo(struct ncclInfo* collInfo) {
  ...
  if (collInfo->recvbuff != nullptr) {
    auto& collInfo_entry = static_cast<struct ncclMockOutput *>(collInfo->recvbuff)->collInfos[collInfo->coll][collInfo->nBytes];
    collInfo_entry.nchannels = collInfo->nChannels;
  }

  INFO(NCCL_MOCK_ROOT, "collInfo->nChannels: %d, collInfo->nThreads: %d", collInfo->nChannels, collInfo->nThreads);
  return ncclSuccess;
}
...
// Calculate chunk and step information for collective communication operations.
//
// Parameters:
// struct ncclInfo* collInfo - Contains communication information such as protocol, algorithm, data type, and communication context.
// size_t nBytes             - Number of bytes to be transmitted.
// int nChannels             - Number of channels used for communication.
//
// Return value:
// ncclResult_t              - Returns the result of the operation, if successful return ncclSuccess.
static ncclResult_t computeCollChunkInfo(struct ncclInfo* collInfo, size_t nBytes, int nChannels) {
  int stepSize = collInfo->comm->buffSizes[collInfo->protocol] / NCCL_STEPS;
  int chunkSteps = (collInfo->protocol == NCCL_PROTO_SIMPLE && collInfo->algorithm == NCCL_ALGO_RING) ? collInfo->chunkSteps : 1; // collInfo->chunkSteps come from collectives.cc ncclInfo, and will change in Protocol
  ...
  INFO(NCCL_MOCK_ROOT, "After computeCollChunkInfo nBytes: %lu, nChannel: %d, collInfo: chunkSize(%d), chunkCount(%d), chunkSteps(%d), sliceSteps(%d), stepSize(%d).", 
                      nBytes, nChannels, collInfo->chunkSize, collInfo->chunkCount, chunkSteps, sliceSteps, stepSize);
  return ncclSuccess;
}
