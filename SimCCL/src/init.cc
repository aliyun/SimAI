...
static ncclResult_t fillInfo(struct ncclComm* comm, struct ncclPeerInfo* info, uint64_t commHash) {
  ...
  info->gdrSupport = 1; // Will be set from Xml
  info->comm = comm;
...
#define DEFAULT_LL_BUFFSIZE (NCCL_LL_LINES_PER_THREAD*NCCL_LL_MAX_NTHREADS*NCCL_STEPS*sizeof(union ncclLLFifoLine)) /* 512KiB */
#define DEFAULT_LL128_BUFFSIZE (NCCL_LL128_ELEMS_PER_THREAD*NCCL_LL128_MAX_NTHREADS*NCCL_STEPS*sizeof(uint64_t)) /* 4800KiB */
...
extern char *name_proto[NCCL_NUM_PROTOCOLS];
...
static ncclResult_t computeBuffSizes(struct ncclComm* comm) {
  ...
  for (int p=0; p<NCCL_NUM_PROTOCOLS; p++) {
    ...
    INFO(NCCL_MOCK_ROOT, "comm->buffSizes[%s]: %d", name_proto[p], comm->buffSizes[p]);
  }
  ...
  INFO(NCCL_MOCK_ROOT, "comm->p2pChunkSize: %d", comm->p2pChunkSize);
...
static void PrintPeerInfo(struct ncclPeerInfo *info, int index) {
  if (info == nullptr) return;
  char busId[NVML_DEVICE_PCI_BUS_ID_BUFFER_SIZE];
  int64ToBusId(info->busId, busId);
  INFO(NCCL_MOCK_ROOT, "info_index(%d): %p, rank: %d, cudaDev: %d, nvmlDev: %d, gdrSupport: %d,"
      "hostHash: %" PRIu64 ", pidHash: %" PRIu64 ", busId: %s, cudaCompCap: %d.", index, info, info->rank, info->cudaDev,
      info->nvmlDev, info->gdrSupport, info->hostHash, info->pidHash, busId, info->cudaCompCap);
  return;
}

static void PrintComm(struct ncclComm *comm, int index) {
  if (comm == nullptr) return;
  INFO(NCCL_MOCK_ROOT, "comm_index(%d): %p, node: %d, nNodes: %d, localRank: %d, localRanks: %d, "
       "maxLocalRanks: %d, intraRank: %d, intraRanks: %d, ncclCollNet: %d.", index, comm, comm->node, comm->nNodes, comm->localRank, comm->localRanks,
       comm->maxLocalRanks, comm->intraRank, comm->intraRanks, comm->ncclCollNet != nullptr ? 1 : 0);
  return;
}
...
static void PrintTopoRanks(struct ncclTopoRanks *topoRanks, int myrank) {
  if (topoRanks == nullptr) return;
  INFO(NCCL_MOCK, "Rank %d topoRanks info: ", myrank);
  INFO(NCCL_MOCK, "ringRecv:");
  printIntArray(topoRanks->ringRecv, MAXCHANNELS);
  INFO(NCCL_MOCK, "ringSend:");
  printIntArray(topoRanks->ringSend, MAXCHANNELS);
  INFO(NCCL_MOCK, "ringPrev:");
  printIntArray(topoRanks->ringPrev, MAXCHANNELS);
  INFO(NCCL_MOCK, "ringNext:");
  printIntArray(topoRanks->ringNext, MAXCHANNELS);
  INFO(NCCL_MOCK, "treeToParent:");
  printIntArray(topoRanks->treeToParent, MAXCHANNELS);
  INFO(NCCL_MOCK, "treeToChild0:");
  printIntArray(topoRanks->treeToChild0, MAXCHANNELS);
  INFO(NCCL_MOCK, "treeToChild1:");
  printIntArray(topoRanks->treeToChild1, MAXCHANNELS);
  INFO(NCCL_MOCK, "nvlsHeads:");
  printIntArray(topoRanks->nvlsHeads, MAXCHANNELS);
  INFO(NCCL_MOCK, "nvlsHeadNum: %d", topoRanks->nvlsHeadNum);
  return;
}
...
// default -1, Enable with Mock pattern
// ncclParamNumMockGpu() number of Mocked GPU
NCCL_PARAM(NumMockGpu, "NUM_MOCK_GPU", 0);
// ncclParamNumMockNode() number of Mocked Node
NCCL_PARAM(NumMockNode, "NUM_MOCK_NODE", 0);
// Mock for nvls
NCCL_PARAM(MNvlsEnable, "NVLS_ENABLE", 2);

// #define NO_MOCK
// #define MOCK_DEBUG
#define MOCK_MAX_NODES 1024
static ncclResult_t initTransportsRank(struct ncclComm* comm, struct ncclComm* parent = NULL) {
  comm->mockOutput->mockName = "MockNccl v2.20-5";
  // We use 2 AllGathers
  ...
  struct ncclTopoGraph *mock_ringGraph = NULL;
  struct ncclTopoGraph *mock_treeGraph = NULL;
  struct ncclTopoGraph *mock_nvlsGraph = NULL;
  struct ncclTopoGraph *mock_graphs[MOCK_MAX_NODES][NCCL_NUM_ALGORITHMS];
  struct ncclComm *mock_comm = NULL;
  ...
  int tmp_minCompCap = 9999, tmp_maxCompCap = 0;
  int nChannelsOrig;
  int *mock_nChannelsOrig;
  struct allGatherInfo *allGather3Data = NULL;
  struct allGatherInfo *mock_allGather3Data = NULL;
  struct ncclTopoRanks** allTopoRanks = NULL;
  struct ncclTopoRanks** mock_allTopoRanks = NULL;
  int *nodesFirstRank = NULL, *nodesTreePatterns = NULL; // nodesFirstRank[nNodes] first rank of every node from ringRecv[0]
  int *rings = NULL;
  int **mock_rings = NULL;
  int* nvbPeers = NULL;
  ...
  int mock_nNodes = ncclParamNumMockNode();
  int mock_nRanks = ncclParamNumMockGpu();
  int mock_nRanksPerNode = mock_nRanks / mock_nNodes;

  nranks = mock_nRanks;
  comm->nRanks = mock_nRanks;
  INFO(NCCL_MOCK_ROOT, "Mocking start: mock_nNodes(%d), mock_nRanks(%d), mock_nRanksPerNode(%d).", mock_nNodes, mock_nRanks, mock_nRanksPerNode);


  // Init Mock ncclComm
  NCCLCHECKGOTO(ncclCalloc(&mock_comm, mock_nRanks), ret, fail);
  for (int i = 0; i < mock_nRanks; i++) {
    mock_comm[i].rank = i;
    mock_comm[i].nRanks = mock_nRanks;
    mock_comm[i].nNodes = mock_nNodes;
    mock_comm[i].mockOutput = comm->mockOutput;
    struct ncclSharedResources* sharedRes = NULL;
    NCCLCHECK(ncclCalloc(&sharedRes, 1));
    /* most of attributes are assigned later in initTransportsRank(). */
    sharedRes->owner = &mock_comm[i];
    sharedRes->tpNRanks = mock_comm[i].nRanks;
    NCCLCHECK(ncclCalloc(&sharedRes->tpRankToLocalRank, mock_comm[i].nRanks));
    // CUDA related
    // NCCLCHECK(ncclStrongStreamConstruct(&sharedRes->deviceStream));
    // NCCLCHECK(ncclStrongStreamConstruct(&sharedRes->hostStream));
    mock_comm[i].sharedRes = sharedRes;
    sharedRes->refCount = 1;
  }

  // AllGather1 - begin
  NCCLCHECKGOTO(ncclCalloc(&comm->peerInfo, nranks+1), ret, fail); // Extra rank to represent CollNet root
  // TODO Mock ncclComm fill peerInfo
  NCCLCHECKGOTO(fillInfo(comm, comm->peerInfo+rank, comm->commHash), ret, fail);
  NCCLCHECKGOTO(bootstrapAllGather(comm->bootstrap, comm->peerInfo, sizeof(struct ncclPeerInfo)), ret, fail);
  if (comm->rank != 0) return ncclSuccess; // Fix multi thread segment fault

  PrintPeerInfo(comm->peerInfo, 0);
  // PrintPeerInfo(comm->peerInfo+7, 7);
  // PrintPeerInfo(comm->peerInfo+nranks-1, nranks-1);
  PrintComm(comm, 0);

  for (int i = 0; i < nranks; i++) {
    // ranks have the same hostHash within the same node.
    if (comm->peerInfo[i].hostHash != comm->peerInfo[rank].hostHash) nNodes++; //  nNodes represents the number of ranks that are not on the same node as the current rank.
...
#include "cudawrap.h"
  if (comm->rank == 0)
    INFO(NCCL_MOCK_ROOT, "nNodes: %d.", nNodes);
  // MNNVL(多机NVLINK) support
  if (nNodes > 1) {
    int cliqueSize = 0;
    comm->MNNVL = 0;
  ...
  do {
    ...
    comm->intraRank = intraProcRank; // The rank number in the current process，0 under MultiDevPerP.
    comm->intraRanks = intraProcRanks; // number of rank in single process, 1 under MultiDevPerP
  ...
  NCCLCHECKGOTO(ncclTopoGetSystem(comm, &comm->topo), ret, fail); // gpu index in topo is global index
  //TODO Per node per topo.xml
  for (int r = 0; r < mock_nRanks; r++)
    NCCLCHECKGOTO(ncclTopoGetSystem(mock_comm+r, &mock_comm[r].topo), ret, fail);
  // Compute paths between GPUs and NICs
  NCCLCHECKGOTO(ncclTopoComputePaths(comm->topo, comm), ret, fail);
  for (int r = 0; r < mock_nRanks; r++)
    NCCLCHECKGOTO(ncclTopoComputePaths((mock_comm+r)->topo, mock_comm+r), ret, fail);
  // Remove inaccessible GPUs and unused NICs
  // If system->nodes[GPU].count == comm->nRanks, then delete NETnode
  comm->nNodes = mock_nNodes;
  NCCLCHECKGOTO(ncclTopoTrimSystem(comm->topo, comm), ret, fail);
  comm->nNodes = 0;
  for (int r = 0; r < mock_nRanks; r++) {
    NCCLCHECKGOTO(ncclTopoTrimSystem((mock_comm+r)->topo, mock_comm+r), ret, fail);
  }
  ...
  // Example: GPU/AD000 :GPU/13000 (2/240.000000/NVL) GPU/19000 (2/240.000000/NVL) GPU/48000 (2/240.000000/NVL) GPU/4D000 (2/240.000000/NVL) GPU/89000 (2/240.000000/NVL) GPU/8E000 (2/240.000000/NVL) GPU/AD000 (0/5000.000000/LOC) GPU/B3000 (2/240.000000/NVL) NVS/0 (1/240.000000/NVL) CPU/0 (4/10.000000/SYS) CPU/1 (3/24.000000/PHB) NET/0 (6/10.000000/SYS) NET/1 (6/24.000000/PXN) NET/2 (6/24.000000/PXN) NET/3 (6/24.000000/PXN) NET/4 (4/24.000000/PXB)
  NCCLCHECKGOTO(ncclTopoPrint(comm->topo), ret, fail);

  // // Mock topo get, now only get GPU/NET... count
  // for (int i = 0; i < mock_nRanks; i++) {
  //   // NCCLCHECKGOTO(ncclTopoGetSystem(&mock_comm[i], &mock_comm[i].topo), ret, fail);
  //   mock_comm[i].topo = comm->topo;
  // }

  for (int r = 0; r < mock_nRanks; r++) {
    tmp_minCompCap = std::min(mock_comm[r].minCompCap, tmp_minCompCap);
    tmp_maxCompCap = std::max(mock_comm[r].maxCompCap, tmp_maxCompCap);
  }
  for (int r = 0; r < mock_nRanks; r++) {
    mock_comm[r].minCompCap = tmp_minCompCap;
    mock_comm[r].maxCompCap = tmp_maxCompCap;
  }

  INFO(NCCL_MOCK_ROOT, "comm->compCap: %d, comm->minCompCap: %d, comm->maxCompCap: %d", comm->compCap, comm->minCompCap, comm->maxCompCap);
  INFO(NCCL_MOCK_ROOT, "mock_comm[0].compCap: %d, mock_comm[0].minCompCap: %d, mock_comm[0].maxCompCap: %d", mock_comm[0].compCap, mock_comm[0].minCompCap, mock_comm[0].maxCompCap);
  ...
  comm->nvlsSupport = (ncclParamMNvlsEnable() == 1) ? 1 : 0;

  for (int r = 0; r < mock_nRanks; r++) {
    mock_comm[r].nvlsSupport = comm->nvlsSupport;
  }
  ...
  NCCLCHECKGOTO(ncclTopoCompute(comm->topo, &ringGraph), ret, fail); // gpu global rank
  NCCLCHECKGOTO(ncclTopoPrintGraph(comm->topo, &ringGraph), ret, fail);
  // Example: 0 : NET/1 GPU/0 GPU/7 GPU/6 GPU/5 GPU/4 GPU/3 GPU/1 GPU/2 NET/2
  ...
  // Mock Node TopoGraph Create only on rank0
  NCCLCHECKGOTO(ncclCalloc(&mock_ringGraph, mock_nNodes), ret, fail);
  NCCLCHECKGOTO(ncclCalloc(&mock_treeGraph, mock_nNodes), ret, fail);
  NCCLCHECKGOTO(ncclCalloc(&mock_nvlsGraph, mock_nNodes), ret, fail);
  for (size_t i = 0; i < mock_nNodes; i++) {
    // TODO Now only support same TopoGraph
    memcpy(&mock_ringGraph[i], &ringGraph, sizeof(struct ncclTopoGraph));
    memcpy(&mock_treeGraph[i], &treeGraph, sizeof(struct ncclTopoGraph));
    memcpy(&mock_nvlsGraph[i], &nvlsGraph, sizeof(struct ncclTopoGraph));
    // Rerank GPU intra_rank -> global_rank, do not rerank when supporting mock_comm
    // Mock Ring
    for (size_t c = 0; c < mock_ringGraph[i].nChannels; c++) {
      for (size_t g = 0; g < mock_nRanksPerNode; g++) {
        mock_ringGraph[i].intra[mock_nRanksPerNode*c+g] += mock_nRanksPerNode*i;
      }
    }
    // Mock Tree
    for (size_t c = 0; c < mock_treeGraph[i].nChannels; c++) {
      for (size_t g = 0; g < mock_nRanksPerNode; g++) {
        mock_treeGraph[i].intra[mock_nRanksPerNode*c+g] += mock_nRanksPerNode*i;
      }
    }
    // Mock NVLS Tree (Only first rank =/= 0 as shown)
    if (mock_comm[i*mock_nRanksPerNode].nvlsSupport == 1) {
      for (int c = 0; c < mock_nvlsGraph[i].nChannels; c++) {
        mock_nvlsGraph[i].intra[mock_nRanksPerNode*c] += mock_nRanksPerNode*i;
      }
    }
    INFO(NCCL_MOCK_ROOT, "------ Mock ring Graph[node: %ld]: ------", i);
    NCCLCHECKGOTO(ncclTopoPrintGraph(mock_comm[i * mock_nRanksPerNode].topo, &mock_ringGraph[i]), ret, fail);
    INFO(NCCL_MOCK_ROOT, "-------------------------------\n");
    INFO(NCCL_MOCK_ROOT, "------ Mock tree Graph[node: %ld]: ------", i);
    NCCLCHECKGOTO(ncclTopoPrintGraph(mock_comm[i * mock_nRanksPerNode].topo, &mock_treeGraph[i]), ret, fail);
    INFO(NCCL_MOCK_ROOT, "-------------------------------\n");
    if (mock_comm[i*mock_nRanksPerNode].nvlsSupport == 1) {
      INFO(NCCL_MOCK_ROOT, "------ Mock nvls tree Graph[node: %ld]: ------", i);
      NCCLCHECKGOTO(ncclTopoPrintGraph(mock_comm[i * mock_nRanksPerNode].topo, &mock_nvlsGraph[i]), ret, fail);
      INFO(NCCL_MOCK_ROOT, "-------------------------------\n");
    }
  }
  // Mock graphs Init
  for (int n = 0; n < mock_nNodes; n++) {
    mock_graphs[n][NCCL_ALGO_TREE] = &mock_treeGraph[n];
    mock_graphs[n][NCCL_ALGO_RING] = &mock_ringGraph[n];
    mock_graphs[n][NCCL_ALGO_COLLNET_DIRECT] = &collNetGraph;
    mock_graphs[n][NCCL_ALGO_COLLNET_CHAIN] = &collNetGraph;
    mock_graphs[n][NCCL_ALGO_NVLS] = &mock_nvlsGraph[n];
    mock_graphs[n][NCCL_ALGO_NVLS_TREE] = &mock_nvlsGraph[n];
  }
  ...
  // mock_allGather3Data Create only on rank0
  if (comm->rank == 0 && mock_nRanks > 1) {
    NCCLCHECKGOTO(ncclCalloc(&mock_allGather3Data, mock_nRanks), ret, fail);
    for (int n = 0; n < mock_nRanks; n++) {
      for (int a=0; a<NCCL_NUM_ALGORITHMS; a++) {
        int nodeIdx = n / mock_nRanksPerNode;
        if (a != NCCL_ALGO_COLLNET_DIRECT && a != NCCL_ALGO_COLLNET_CHAIN) {
          mock_allGather3Data[n].graphInfo[a].pattern = mock_graphs[nodeIdx][a]->pattern;
          mock_allGather3Data[n].graphInfo[a].nChannels = mock_graphs[nodeIdx][a]->nChannels;
          mock_allGather3Data[n].graphInfo[a].sameChannels = mock_graphs[nodeIdx][a]->sameChannels;
          mock_allGather3Data[n].graphInfo[a].bwIntra = mock_graphs[nodeIdx][a]->bwIntra;
          mock_allGather3Data[n].graphInfo[a].bwInter = mock_graphs[nodeIdx][a]->bwInter;
          mock_allGather3Data[n].graphInfo[a].typeIntra = mock_graphs[nodeIdx][a]->typeIntra;
          mock_allGather3Data[n].graphInfo[a].typeInter = mock_graphs[nodeIdx][a]->typeInter;
        }
      }
      mock_comm[n].nChannels = std::min(mock_treeGraph[n/mock_nRanksPerNode].nChannels, mock_ringGraph[n/mock_nRanksPerNode].nChannels);
      NCCLCHECKGOTO(ncclTopoPreset(&mock_comm[n], mock_graphs[n/mock_nRanksPerNode], &mock_allGather3Data[n].topoRanks), ret, fail);
    }
  }
...
#ifdef MOCK_DEBUG
  if (rank == 0) {
    INFO(NCCL_MOCK_ROOT, "------ Before TopoRanks[%d]: ------", rank);
    for (int i = 0; i < nranks; i++) {
      PrintTopoRanks(&allGather3Data[i].topoRanks, i);
    }
    INFO(NCCL_MOCK_ROOT, "----------------------------");
  }
#endif
  // NCCLCHECKGOTO(bootstrapAllGather(comm->bootstrap, allGather3Data, sizeof(*allGather3Data)), ret, fail);
  memcpy(allGather3Data, mock_allGather3Data, sizeof(allGatherInfo) * mock_nRanks);
  // allGather3Data = mock_allGather3Data;
#ifdef MOCK_DEBUG
  if (rank == 0) {
    INFO(NCCL_MOCK_ROOT, "------ After TopoRanks[%d]: ------", rank);
    for (int i = 0; i < mock_nRanks; i++) {
      PrintTopoRanks(&mock_allGather3Data[i].topoRanks, i);
    }
    INFO(NCCL_MOCK_ROOT, "----------------------------");
  }
#endif
  ...
  for (int r=0; r<nranks; r++) {
    // Here, based on the ringRecv[0] of each rank obtained from allGather,
    // we determine the total number of comm->nNodes
    // (the judgment is based on the fact that the ringRecv[0] of different ranks from the same node are the same).
    int node;
  ...
  for (int r = 0; r < mock_nRanks; r++) {
    NCCLCHECKGOTO(ncclCalloc(&mock_comm[r].rankToNode, mock_comm[r].nRanks), ret, fail);
    memcpy(mock_comm[r].rankToNode, comm->rankToNode, sizeof(int) * mock_comm[r].nRanks);
  }

#ifdef MOCK_DEBUG
  for (int i = 0; i < nranks; i++) {
    INFO(NCCL_MOCK_ROOT, "nodesFirstRank[%d]: %d", i, nodesFirstRank[i]);
    INFO(NCCL_MOCK_ROOT, "mock_comm[15].rankToNode[%d]: %d", i, mock_comm[15].rankToNode[i]);
  }
#endif

  INFO(NCCL_MOCK_ROOT, "comm->nNodes: %d", comm->nNodes);
  ...
  for (int r = 0; r < mock_nRanks; r++) {
    // Now that we know mock_nNodes, alloc mock nodeRanks and compute mock localRanks for each mock node
    NCCLCHECKGOTO(ncclCalloc(&mock_comm[r].nodeRanks, mock_comm[r].nNodes), ret, fail);
    NCCLCHECKGOTO(ncclCalloc(&mock_comm[r].rankToLocalRank, mock_comm[r].nRanks), ret, fail);

    for (int i=0; i<mock_comm[r].nRanks; i++) {
      int node = mock_comm[r].rankToNode[i];
      mock_comm[r].rankToLocalRank[i] = mock_comm[r].nodeRanks[node].localRanks;
      mock_comm[r].nodeRanks[node].localRanks++;
    }
    // Allocate mock ranks arrays for each mock node
    for (int n=0; n<mock_comm[r].nNodes; n++) {
      NCCLCHECKGOTO(ncclCalloc(&mock_comm[r].nodeRanks[n].localRankToRank, mock_comm[r].nodeRanks[n].localRanks), ret, fail);
      mock_comm[r].maxLocalRanks = std::max(mock_comm[r].maxLocalRanks, mock_comm[r].nodeRanks[n].localRanks);
      mock_comm[r].nodeRanks[n].localRanks = 0;
    }
    // And fill the mock ranks arrays
    for (int j=0; j<mock_comm[r].nRanks; j++) {
      int node = mock_comm[r].rankToNode[j];
      mock_comm[r].nodeRanks[node].localRankToRank[mock_comm[r].nodeRanks[node].localRanks++] = j;
    }
    mock_comm[r].node = mock_comm[r].rankToNode[r];
    mock_comm[r].localRankToRank = mock_comm[r].nodeRanks[mock_comm[r].node].localRankToRank;
    mock_comm[r].localRank = mock_comm[r].rankToLocalRank[r];
    mock_comm[r].localRanks = mock_comm[r].nodeRanks[mock_comm[r].node].localRanks;
    // INFO(NCCL_MOCK_ROOT, "mock_comm[%d].node: %d", r, mock_comm[r].node);
  }
  ...
  nChannelsOrig = comm->nChannels; // 此时还没有 double
  NCCLCHECKGOTO(ncclCalloc(&mock_nChannelsOrig, mock_nRanks), ret, fail);
  for (int r = 0; r < mock_nRanks; r++) {
    mock_nChannelsOrig[r] = mock_comm[r].nChannels;
  }
  NCCLCHECKGOTO(ncclCalloc(&allTopoRanks, comm->nRanks), ret, fail);
  NCCLCHECKGOTO(ncclCalloc(&mock_allTopoRanks, mock_comm->nRanks), ret, fail);
  for (int i=0; i<nranks; i++) {
    allTopoRanks[i] = &allGather3Data[i].topoRanks;
    mock_allTopoRanks[i] = &mock_allGather3Data[i].topoRanks;
    ...
    for (int a=0; a<NCCL_NUM_ALGORITHMS; a++) {
      ...
      mock_graphs[i/mock_nRanksPerNode][a]->nChannels = std::min(allGather3Data[i].graphInfo[a].nChannels, graphs[a]->nChannels);
      mock_graphs[i/mock_nRanksPerNode][a]->sameChannels = std::min(allGather3Data[i].graphInfo[a].sameChannels, graphs[a]->sameChannels);
      mock_graphs[i/mock_nRanksPerNode][a]->bwIntra = std::min(allGather3Data[i].graphInfo[a].bwIntra, graphs[a]->bwIntra);
      mock_graphs[i/mock_nRanksPerNode][a]->bwInter = std::min(allGather3Data[i].graphInfo[a].bwInter, graphs[a]->bwInter);
      mock_graphs[i/mock_nRanksPerNode][a]->typeIntra = std::max(allGather3Data[i].graphInfo[a].typeIntra, graphs[a]->typeIntra);
      mock_graphs[i/mock_nRanksPerNode][a]->typeInter = std::max(allGather3Data[i].graphInfo[a].typeInter, graphs[a]->typeInter);
    }
    ...
    if (mock_graphs[i/mock_nRanksPerNode][NCCL_ALGO_COLLNET_CHAIN]->nChannels == 0) mock_comm[i/mock_nRanksPerNode].collNetSupport = 0;
    if (mock_graphs[i/mock_nRanksPerNode][NCCL_ALGO_NVLS]->nChannels == 0) mock_comm[i/mock_nRanksPerNode].nvlsSupport = 0;
  }

  INFO(NCCL_MOCK_ROOT, "treeGraph.nChannels: %d, ringGraph.nChannels: %d", treeGraph.nChannels, ringGraph.nChannels);
  ...
  for (int r = 0; r < mock_nRanks; r++) {
    mock_comm[r].nChannels = treeGraph.nChannels = ringGraph.nChannels = std::min(treeGraph.nChannels, ringGraph.nChannels);
    if (mock_comm[r].nChannels < mock_nChannelsOrig[r]) {
      // We started duplicating channels during Preset(), so we need to move the
      // duplicated channels since we have removed some.
      for (int i=0; i<mock_comm[r].nChannels; i++) memcpy(mock_comm[r].channels+mock_comm[r].nChannels+i, mock_comm[r].channels+mock_nChannelsOrig[r]+i, sizeof(struct ncclChannel));
    }
  }
  ...
  for (int r = 0; r < mock_nRanks; r++) {
    mock_comm[r].config.maxCTAs = 32;
    mock_comm[r].config.minCTAs = 1;
  }
  NCCLCHECKGOTO(ncclCalloc(&rings, nranks*MAXCHANNELS), ret, fail);
  NCCLCHECKGOTO(ncclTopoPostset(comm, nodesFirstRank, nodesTreePatterns, allTopoRanks, rings, graphs), ret, fail); // Double channels
  // AllGather3 - end

  INFO(NCCL_MOCK_ROOT, "mock_comm->config.maxCTAs: %d, mock_comm->config.minCTAs: %d, mock_comm->sharedRes->tpNChannels: %d", mock_comm->config.maxCTAs, mock_comm->config.minCTAs, mock_comm->sharedRes->tpNChannels);
  NCCLCHECKGOTO(ncclCalloc(&mock_rings, mock_nRanks), ret, fail);
  // Mock comms also need ncclTopoPostset to connect channel
  for (int r = 0; r < mock_nRanks; r++) {
    NCCLCHECKGOTO(ncclCalloc(&mock_rings[r], nranks*MAXCHANNELS), ret, fail);
    NCCLCHECKGOTO(ncclTopoPostset(mock_comm+r, nodesFirstRank, nodesTreePatterns, mock_allTopoRanks, mock_rings[r], mock_graphs[r/mock_nRanksPerNode]), ret, fail);
  }
  ...
  for (int r = 0; r < mock_nRanks; r++) {
    ...
    line[0]='\0';
    for (int c=0; c<comm->nChannels; c++) {
      struct ncclTree* tree = &mock_comm[r].channels[c].tree;
      snprintf(line+strlen(line), 1023-strlen(line), " [%d] %d/%d/%d->%d->%d",
          c, tree->down[0], tree->down[1], tree->down[2], r, tree->up);
    }
    line[1023] = '\0';
    INFO(NCCL_MOCK_ROOT, "Grank_%d Trees%s", r, line);
  }
  ...
  if (comm->nvlsSupport) {
    for (int r = 0; r < mock_nRanks; r++) {
      ...
      line[0]='\0';
      for (int c=0; c<comm->nChannels; c++) {
        struct ncclNvls* nvlstree = &mock_comm[r].channels[c].nvls;
        snprintf(line+strlen(line), 1023-strlen(line), " [%d] %d/%d/%d->%d->%d",
            c, nvlstree->treeDown[0], nvlstree->treeDown[1], nvlstree->treeDown[2], r, nvlstree->treeUp);
        // INFO(NCCL_MOCK_ROOT, "comm[%d].channels[%d]: out: %d, nHeads: %d, headRank: %d, down: %d, node: %d, nNodes: %d", 
        //       r, c, nvlstree->out, nvlstree->nHeads, nvlstree->headRank, nvlstree->down, nvlstree->node, nvlstree->nNodes);
      }
      line[1023] = '\0';
      //TODO tree is not connected! every mock rank need ncclTopoPostset
      INFO(NCCL_MOCK_ROOT, "Grank_%d NVLS Trees%s", r, line);
    }
  }
  ...
  if (parent && parent->config.splitShare) {
    ...
    //TODO Mock Proxy dumpstates print
  ...
  INFO(NCCL_MOCK_ROOT, "%d coll channels, %d collnet channels, %d nvls channels, %d p2p channels, %d p2p channels per peer", comm->nChannels, comm->collNetChannels, comm->nvlsChannels, comm->p2pnChannels, comm->p2pnChannelsPerPeer);
  //TODO support mockcomm output p2porder
  do {
    for (int mr = 0; mr < mock_nRanks; mr++) {
      struct ncclTasks* tasks = &(mock_comm[mr].tasks);
      int node = mock_comm[mr].node;
      int nNodes = mock_comm[mr].nNodes;
      struct ncclNodeRanks *nodeRanks = mock_comm[mr].nodeRanks;
      int localRank = mock_comm[mr].localRank;
      // We want to fuse along node boundaries. Make sure nsteps is a multiple or divides 8.
      int steps = ALIGN_POWER(mock_comm[mr].maxLocalRanks, NCCL_MAX_WORK_ELEMENTS_P2P/2);
      tasks->p2pOrderSteps = mock_comm[mr].nNodes * steps;
      tasks->peers = ncclMemoryStackAlloc<ncclTasks::Peer>(&mock_comm[mr].memPermanent, tasks->p2pOrderSteps);
      tasks->p2pSendOrder = ncclMemoryStackAlloc<int>(&mock_comm[mr].memPermanent, tasks->p2pOrderSteps);
      tasks->p2pRecvOrder = ncclMemoryStackAlloc<int>(&mock_comm[mr].memPermanent, tasks->p2pOrderSteps);
      int i=0;
...
static ncclResult_t ncclCommInitRankDev(ncclComm_t* newcomm, int nranks, ncclUniqueId commId, int myrank, int cudaDev, ncclConfig_t *config, void *mockOutput) {
  auto *mockOutput_ = static_cast<struct ncclMockOutput *>(mockOutput);
  static_assert(std::is_same<decltype(mockOutput_), struct ncclMockOutput*>::value, "mockOutput must be a pointer to struct ncclMockOutput");
  ncclResult_t res = ncclSuccess;
  ...
  NCCLCHECKGOTO(ncclCalloc((uint32_t**)&comm->abortFlag, 1), res, fail);
...
ncclResult_t ncclCommInitRank(ncclComm_t* newcomm, int nranks, ncclUniqueId commId, int myrank) {
  ...
  INFO(NCCL_MOCK_ROOT, "nranks: %d, myrank: %d, cudaDev: %d.", nranks, myrank, cudaDev);
  NCCLCHECK(ncclCommInitRankDev(newcomm, nranks, commId, myrank, cudaDev, &config, nullptr));
  return ncclSuccess;
}

NCCL_API(ncclResult_t, ncclCommInitAll, ncclComm_t* comms, int ndev, const int* devlist, void *mockOutput);
ncclResult_t ncclCommInitAll(ncclComm_t* comms, int ndev, const int* devlist, void *mockOutput) {
  ncclResult_t ret = ncclSuccess;
  int totalnDev;
  int *gpuFlags = NULL;
  ncclConfig_t config = NCCL_CONFIG_INITIALIZER;
  ...
  for (int i=0; i<ndev; i++) {
    ...
    ncclCommInitRankDev(comms+i, ndev, uniqueId, i, devlist ? devlist[i] : i, &config, mockOutput);
  }
...
ncclResult_t ncclCommInitRankConfig(ncclComm_t *newcomm, int nranks, ncclUniqueId commId, int myrank, ncclConfig_t *config) {
  ...
  NCCLCHECKGOTO(ncclCommInitRankDev(newcomm, nranks, commId, myrank, cudaDev, internalConfigPtr, nullptr), ret, fail);
