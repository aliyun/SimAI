 ...
 *
 * ---
 * DECOUPLED REPLAY: Topology configuration and network setup.
 * Extracted from: astra-sim-alibabacloud/astra-sim/network_frontend/ns3/common.h
 *
 * Functions extracted:
 *   ReadConf()         - common.h:482-687
 *   SetConfig()        - common.h:690-714
 *   SetupNetwork()     - common.h:716-1064
 *   CalculateRoute()   - common.h:264-322
 *   CalculateRoutes()  - common.h:324-330
 *   SetRoutingEntries()- common.h:332-361
 *   get_nic_rate()     - common.h:474-480
 *   get_pfc()          - common.h:175-179
 *   get_output_file_name() - common.h:468-472
 *   TakeDownLink()     - common.h:444-466
 *   printRoutingEntries()  - common.h:363-438
 *   schedule_monitor() - common.h:238-262
 *   monitor_*()        - common.h:193-237
 *
 * Removed from SimAI version:
 *   - No MockNcclLog references
 *   - #if ENABLE_QP guard removed (always enabled for decoupled replay)
 *   - Hardcoded Linux paths replaced with local defaults
 */

#ifndef __DECOUPLED_TOPOLOGY_READER_H__
#define __DECOUPLED_TOPOLOGY_READER_H__

#include "common_types.h"

// ============================================================================
// get_output_file_name
// Extracted from: common.h:468-472
// ============================================================================

// Extracted from: common.h:468-472
inline std::string get_output_file_name(std::string config_file, std::string output_file) {
    auto idx = config_file.find_last_of('/');
    std::string ans = output_file.substr(0, output_file.length() - 4) + config_file.substr(idx + 7);
    return ans;
}

// ============================================================================
// get_nic_rate
// Extracted from: common.h:474-480
// ============================================================================

// Extracted from: common.h:474-480
inline uint64_t get_nic_rate(NodeContainer &n) {
...
// ============================================================================
// monitor_* functions
// Extracted from: common.h:193-237
// ============================================================================

// Extracted from: common.h:193-204
inline void monitor_qlen(FILE* qlen_output, NodeContainer *n) {
    for (uint32_t i = 0; i < n->GetN(); i++) {
...
// Extracted from: common.h:205-219
inline void monitor_bw(FILE* bw_output, NodeContainer *n) {
    for (uint32_t i = 0; i < n->GetN(); i++) {
        if (n->Get(i)->GetNodeType() == 1) {
            Ptr<SwitchNode> sw = DynamicCast<SwitchNode>(n->Get(i));
...
// Extracted from: common.h:220-228
inline void monitor_qp_rate(FILE* rate_output, NodeContainer *n) {
    for (uint32_t i = 0; i < n->GetN(); i++) {
        if (n->Get(i)->GetNodeType() == 0) {
            Ptr<Node> host = n->Get(i);
...
// Extracted from: common.h:229-237
inline void monitor_qp_cnp_number(FILE* cnp_output, NodeContainer *n) {
    for (uint32_t i = 0; i < n->GetN(); i++) {
        if (n->Get(i)->GetNodeType() == 0) {
            Ptr<Node> host = n->Get(i);
...
// Extracted from: common.h:238-262
inline void schedule_monitor() {
...
// ============================================================================
// CalculateRoute
// Extracted from: common.h:264-322
// ============================================================================

// Extracted from: common.h:264-322
inline void CalculateRoute(Ptr<Node> host) {
    vector<Ptr<Node>> q;
...
// ============================================================================
// CalculateRoutes
// Extracted from: common.h:324-330
// ============================================================================

// Extracted from: common.h:324-330
inline void CalculateRoutes(NodeContainer &n) {
...
// ============================================================================
// SetRoutingEntries
// Extracted from: common.h:332-361
// ============================================================================
...
// ============================================================================
// printRoutingEntries (debug helper)
// Extracted from: common.h:363-438
// ============================================================================

// Extracted from: common.h:363-438
inline void printRoutingEntries() {
    ...
    for (auto it = NetSwitch.begin(); it != NetSwitch.end(); ++it) {
        ...
        for (auto j = table.begin(); j != table.end(); ++j) {
            ...
            for (auto k = entries.begin(); k != entries.end(); ++k) {
                ...
                cout << "To " << dst->GetId() << "[" << types[dst->GetNodeType()] << "] via "
                     << nextHop->GetId() << "[" << types[nextHop->GetNodeType()] << "]"
                     << " from port: " << interface << endl;
            }
        }
    }
...
// ============================================================================
// validateRoutingEntries
// Extracted from: common.h:440-442
// ============================================================================

// Extracted from: common.h:440-442
inline bool validateRoutingEntries() {
    return false;
}

// ============================================================================
// TakeDownLink
// Extracted from: common.h:444-466
// ============================================================================

// Extracted from: common.h:444-466
inline void TakeDownLink(NodeContainer n, Ptr<Node> a, Ptr<Node> b) {
    if (!nbr2if[a][b].up)
        return;
...
// ============================================================================
// ReadConf
// Extracted from: common.h:482-687
// ============================================================================

// Extracted from: common.h:482-687
inline bool ReadConf(std::string network_topo, std::string network_conf) {
    std::ifstream conf;
...
// ============================================================================
// SetConfig
// Extracted from: common.h:690-714
// ============================================================================
...
// ============================================================================
// SetupNetwork
// Extracted from: common.h:716-1064
// Modified: #if ENABLE_QP guard removed (always enabled)
// ============================================================================

// Extracted from: common.h:716-1064
inline void SetupNetwork(void (*qp_finish)(FILE *, Ptr<RdmaQueuePair>),
    ...
    // RDMA setup (always enabled for decoupled replay)
    {
        ...
        for (uint32_t i = 0; i < node_num; i++) {
            if (n.Get(i)->GetNodeType() == 0 || n.Get(i)->GetNodeType() == 2) {
                ...
                rdma->TraceConnectWithoutContext(
                    "SendComplete", MakeBoundCallback(send_finish, send_output));
            }
        }
    }
...
#endif // __DECOUPLED_TOPOLOGY_READER_H__
