...
def get_fig_for_qp():
    ...
    for key, df in groups:
        ...
        df['curr_rate'] = df['curr_rate'].map(lambda x : x/1e9) # Gbps
        fig.add_scatter(x=df['time'],y=df['curr_rate'], name='flow-'+str(key))
    fig.update_layout(title='Flow Rate', xaxis_title='time(ms)', yaxis_title='Flow Send Rate(Gbps)')
    offline.plot(fig, filename=curr_work_path+'/../simulation/monitor_output/figs/'+file_name+'/flow_rate.html')
    # fig.show()
