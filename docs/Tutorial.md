...
### TOPO Templates
To enhance convenience, we provide 4 templates for common architectures, including Spectrum-X, Single Plane AlibabaHPN, Dual Plane Alibaba HPN and DCN+. The parameter `-topo` can be set.These four figures give an overview of them.
<table>
  <tr>
    <td><img src="./images/Spectrum-X.jpg" alt="Spectrum-X" style="width:100%"></td>
    <p>Spectrum-X(One Pod)</p>
    <td><img src="./images/DCN+.jpg" alt="DCN+" style="width:100%"></td>
    <p>DCN+(One Pod)</p>
  </tr>
  <tr>
    <td><img src="./images/HPNSinglePlane.jpg" alt="AlibabaHPN_SinglePlane" style="width:100%"></td>
    <p>AlibabaHPN with Single Plane(One Pod)</p>
    <td><img src="./images/HPNDualPlane.jpg" alt="AlibabaHPN_DualPlane" style="width:100%"></td>
    <p>AlibabaHPN with Dual Plane(One Pod)</p>
  </tr>
</table>

The following command generates the Spectrum-X topology with 8 GPUs shown in the image:
```bash
python3 ./astra-sim-alibabacloud/inputs/topo/gen_Topo_Template.py -topo Spectrum-X -g 8
```
<img src="./images/Spectrum8gpus.png" alt="Spectrum8gpus" width="30%">

The following table shows the default parameters of each template. Users can change the `-topo` name and corresponding `-g` to generate topology of corresponding structure. Notice that if the gpu num is not enter, then the topology of one Pod will be generated. And if `-g` is less than one host, none asw and psw will be used, for gpu number less than one segment,  none psw will be used. What's more, for users who want to generate AlibabaHPN_Dual_Plane, `--dp` must be added. 
(Gpus more than one pod is not supported for AlibabaHPN and DCN+ right now.)

| Parameter Level | Parameter  | Spectrum-X  | AlibabaHPN_Single_Plane | AlibabaHPN_Dual_Plane | DCN+ | Description |
|-----------------|------------|-------------|-------------------------|-----------------------|------|-------------|
| Whole Structure | `-topo`    | `Spectrum-X`| `AlibabaHPN`            | `AlibabaHPN`          | `DCN+` | Template for topology |
|                 | `-g`       | 4096        | 15360                   | 15360                 | 512  | Number of GPUs |
|                 | `--dp`     | false       | false                   | true                  | false | Enable dual plane, default single plane |
|                 | `--st`     | false       | false                   | false                 | true  | Enable non-rail optimized |
|                 | `-dualToR` | 0           | 1                       | 1                     | 1     | Enable dual nics and dual ToRs |
|                 | `-er`      | 0           | 0                       | 0                     | 0     | Error rate |
| Intra-Host      | `-gps`     | 8           | 8                       | 8                     | 8     | GPU per server |
|                 | `-gt`      | H800        | H800                    | H800                  | H800  | GPU type |
|                 | `-nsps`    | 1           | 1                       | 1                     | 1     | NV switch per server |
|                 | `-nvbw`    | 2400Gbps    | 2400Gbps                | 2400Gbps              | 2400Gbps | NVLink bandwidth |
|                 | `-nl`      | 0.000025ms  | 0.000025ms              | 0.000025ms            | 0.000025ms | NVLink latency |
|                 | `-l`       | 0.0005ms    | 0.0005ms                | 0.0005ms              | 0.0005ms | NIC latency |
| Intra-Segment   | `-bw`      | 400Gbps     | 200Gbps                 | 200Gbps               | 200Gbps | NIC to ASW bandwidth |
|                 | `-asw`      | 64          | 120                     | 120                   | 8     | ASW switch num |
|                 | `-nps`      | 64          | 128                     | 128                   | 128   | NICs per switch |
| Intra-Pod       | `-psn`      | 64          | 120                     | 120                   | 8     | PSW switch num |
|                 | `-apbw`     | 400Gbps     | 400Gbps                 | 400Gbps               | 400Gbps | ASW to PSW bandwidth |
|                 | `-app`      | 64          | 240                     | 120                   | 8     | ASW per PSW |


The user can customize the topology themselves. For example, if you want to construct a single ToR topology with 32 GPUs and 200Gbps bandwidth with A100, you can enter the following command:
```bash
python3 ./astra-sim-alibabacloud/inputs/topo/gen_Topo_Template.py -g 32 -bw 200Gbps -gt A100
```
...
cd SimAI
./scripts/build.sh -c ns3
python3 ./astra-sim-alibabacloud/inputs/topo/gen_Topo_Template.py -g 32 -gt H800 -bw 400Gbps -nvbw 1360Gbps 
AS_SEND_LAT=12 AS_NVLS_ENABLE=1 ./bin/SimAI_simulator -t 8 -w ./example/microAllReduce.txt -n SingleToR_32g_8gps_400Gbps_H800 -c ./astra-sim-alibabacloud/inputs/config/SimAI.conf
```
### RING topo file && RUN
```bash
python3 ./astra-sim-alibabacloud/inputs/topo/gen_Topo_Template.py -g 32 -gt H800 -bw 400Gbps -nvbw 1440Gbps
AS_SEND_LAT=2 AS_PXN_ENABLE=1 ./bin/SimAI_simulator -t 8 -w ./example/microAllReduce.txt -n ./SingleToR_32g_8gps_400Gbps_H800 -c ./astra-sim-alibabacloud/inputs/config/SimAI.conf
```
### result
| msg size | NVLS   | RING   |
|----------|--------|--------|
...
```bash
# DCN+ topofile
python3 ./astra-sim-alibabacloud/inputs/topo/gen_Topo_Template.py -topo DCN+ -g 256
# HPN7.0 topofile
python3 ./astra-sim-alibabacloud/inputs/topo/gen_Topo_Template.py -topo AlibabaHPN -g 256
```
### RUN
```bash
# DCN+ run command
AS_SEND_LAT=2 ./bin/SimAI_simulator -t 8 -w ./example/microAllReduce.txt -n ./DCN+_256g_8gps_200Gbps_H800 -c ./astra-sim-alibabacloud/inputs/config/SimAI.conf
# HPN7.0 run command
AS_SEND_LAT=2 ./bin/SimAI_simulator -t 8 -w ./example/microAllReduce.txt -n ./AlibabaHPN_256g_8gps_SinglePlane_200Gbps_H800 -c ./astra-sim-alibabacloud/inputs/config/SimAI.conf
```
| msg size | HPN 7.0     | DCN       |
|----------|-------------|-----------|
| 16M      | 0.205184    | 0.290613  |
| 32M      | 0.389290    | 0.516217  |
| 64M      | 0.706263    | 0.848813  |
| 128M     | 1.192640    | 1.339161  |
| 256M     | 1.735520    | 1.874856  |
| 512M     | 12.492056   | 11.620479 |
